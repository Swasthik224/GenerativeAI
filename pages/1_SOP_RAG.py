...
# -------------------------------
# 2. Lazy-Loaded Resource Caching
# -------------------------------
@st.cache_resource
def get_embedding_model():
    """Cache only the transformer model framework footprint."""
    return SentenceTransformer("all-MiniLM-L6-v2")


@st.cache_resource
def get_chroma_client():
    """Cache the database client with telemetry disabled to prevent Streamlit crashes."""
...
def show_login_page():
    ...
    with st.form("login_form"):
        ...
        if submit_button:
            ...
            if username_lower in USER_REGISTRY and USER_REGISTRY[username_lower]["password"] == password:
                st.session_state.logged_in = True
                st.session_state.user_role = USER_REGISTRY[username_lower]["role"]
                st.session_state.username = username_input
                st.success(f"Verified! Loading dynamic {st.session_state.user_role} view...")
                st.rerun()
            else:
...
def show_rag_interface():
    ...
    with col1:
        ...
        st.caption(f"Authenticated Identity: **{st.session_state.username}** ({role} Privileges)")
    with col2:
    ...
    if role == "Admin":
        ...
        st.subheader("🛠️ Vector Collection Builder")
        ...
        if uploaded_file and st.button(f"🚀 Initialize and Index Fresh Database for {target_role}s"):
            try:
                with st.spinner("Processing PDF..."):
                    ...
                    collection = chroma_client.create_collection(target_collection)
